    ...
    @if (($background['color'] != 'light-gradient') && ($section['id'] != 'service-card-bg-half') && ($section['id'] != 'service-cards-privacy'))
      ...
      @foreach ($cards as $index => $card)
        ...
        <div class="relative flex-1 sm:basis-6/12 p-3 @if ($columns) lg:basis-{{ $columns }} @else lg:basis-2/6 @endif">
          ...
          @endif
          @if (($section['id'] != 'service-cards-privacy') && $loop->last)
          ...
          <div class="service-icon-card h-full group">
            <div class="bg-white @if ($background['color'] != 'light-gradient') shadow-lg @endif text-brand @if ($card['bg_image']) group-hover:bg-{{ $icon_color_class }} @else group-hover:bg-{{ $icon_color_class }} @endif group-hover:text-white transition-all hover:shadow-lg border border-light relative flex items-start rounded-lg overflow-hidden group h-full">
              ...
              <div class="relative z-10 w-full p-9 text-lg lg:text-4xl">
                ...
                <div class="relative">
                  @isset ($card["icon"])
                    @if (!empty($card["icon"]))
                      <span class="mb-5 bg-light @if ($icon_color_class == 'violet') group-hover:bg-orchid @else group-hover:bg-action-dark @endif w-10 h-10 p-2 flex justify-center items-center rounded-full">
