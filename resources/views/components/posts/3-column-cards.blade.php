  ...
  <div class="component-inner-section @if ($section_settings['fullscreen']) fullscreen @endif">
    ...
    <div>
        ...
        @foreach ($press as $index => $post)
          <div class="relative flex flex-col rounded-lg shadow-lg overflow-hidden">
            ...
            <div class="flex-shrink-0 bg-white border-b-2 border-light">
              <img class="aspect-video w-full object-contain mt-4 p-8 mx-auto" src="{{ $post['thumbnail_url'] }}" alt="{{ $post['thumbnail_alt'] }}">
            </div>
            @endisset
      ...
      @if (count($total_press) > $ppp)
        <div class="mt-14">
          <button type="button" class="inline-flex button button-solid load-more-button loadmore-posts" data-ajax-container="ajax-spotlight" data-ppp="{{ $ppp }}" data-current-page="1" data-template="national-spotlight" data-press-cat="{{ $category }}">
